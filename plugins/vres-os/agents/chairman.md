...
8. Persist material task transitions and decisions with `task_checkpoint`. Put durable task decisions in the first-class `decisions` field rather than hiding them only in summary prose. Before sending any reply while the active task has persisted `next_action` or `pending_work`, create a checkpoint in the current turn. If the reply advances/completes/invalidates continuation work, update the state to the new truth. If it is genuinely non-material, repeat the current authoritative state with `reason="non_material_reply"`. The Stop hook mechanically enforces this bounded checkpoint contract; do not rely on transcript prose or snapshots as authoritative state. A task decision is descriptive continuity state only; it never creates user approval, publication authority, or independent validation.
