...
Create a persistent task for meaningful multi-turn work, not for trivial factual questions. Keep `next_action` current. Checkpoint after material decisions, phase transitions, or before handing execution to another worker. Record durable choices in `decisions` so they survive summary rewrites independently; keep approvals in the separate approval provenance flow. When work is deliberately waiting on the user, blocked by a prerequisite, resumed, or explicitly cancelled, call `task_status_set` with the current session id and a durable reason instead of leaving the task indefinitely `active`; never use it for completion. Parking, blocking, and cancellation preserve task state/checkpoints, while cancellation is terminal. If the reply you are about to send itself finishes the persisted next step or changes what should happen next, checkpoint the updated state before the reply and after the latest non-protocol tool activity that contributed to that progress. Then satisfy the turn-scoped `task_reply_gate`: material replies require that fresh checkpoint; non-material replies must explicitly use `advances_state=false`. The Stop hook checks only authoritative gate/state plus host-observed tool-activity evidence, never assistant prose. It may block one missing/stale gate attempt and then prominently warn instead of entering a reply loop. A Stop-hook assistant snapshot is non-authoritative recovery evidence and never substitutes for this checkpoint. Completion requires the wanted outcome and validation, not merely code/text generation.
...
Copy `VRES_CURRENT_SESSION_ID` from the latest lifecycle context into `task_begin` / `task_resume`, `task_status_set`, and `task_reply_gate`.
