...
5. For meaningful persistent work that needs expertise, route through the Fable routing governor. The governor, not you, decides the smallest competent team and Sonnet-vs-Opus execution tier.
...
For meaningful persistent work that needs routed expertise, make routing mechanically inspectable and Fable-governed.
...
2. Immediately call `routing_prepare` for that discovery and include every applicable risk trigger. `acceptance_test`, release/security/provenance/governance, high-stakes/regulated work, capability/procedure/model proof/promotion, company-wide publication, irreversible external actions, durable material disagreement, or explicit protected-review requests are hard protected triggers. Do not omit a real trigger to obtain a cheaper route.
3. Delegate the exact returned routing request/context/instruction to `vres-os:routing-arbiter`. This must be the returned Fable/high governor; never substitute Sonnet, Opus, or an unnamed generic agent. After it stops, call `routing_result` and use only the host-observed recorded verdict.
4. If the governor returns `blocked`, treat its `required_gap_needs` as real capability gaps. Acquire only project-scoped expertise with `capability_acquire_project`, concrete acquisition provenance, and an explicit `specialist-`/`specialist:` owner; then rediscover and obtain a fresh Fable route. Registration is not proof. Do not acquire a specialist before the Fable governor has confirmed the discovery gap.
5. If the governor returns `routed`, call `orchestration_plan_record` using exactly its lead role, selected roles, covers and capability keys. Add the required explicit exclusions for every stable routable role not selected. Do not add an expert, change the lead, or swap capability ownership after routing. If the route needs to change, prepare a fresh Fable route instead.
6. Execute each routed expert on the exact tier selected by Fable. For `execution_tier="sonnet"`, delegate to `vres-os:sonnet-expert`; for `execution_tier="opus"`, delegate to `vres-os:opus-expert`. Supply task key, current session id, plan key, selected role, covered needs, capability keys, objective and bounded assignment. The worker must call `orchestration_expert_report` itself before returning. Do not use the old role-specific/inherited-model agents as substitutes for governed work, because their actual execution tier is not the routed contract. Host-observed worker model evidence is authoritative; worker prose about its model is not.
7. Preserve disagreement. When selected experts materially conflict, call `orchestration_arbitrate` with the conflicting reports and Challenger report when one was routed. If a consequential/durable disagreement appears that was not represented in the current risk triggers/route, return to `routing_prepare` with `material_durable_disagreement`; do not silently finish under a cheaper assurance contract.
8. After every routed expert is accounted for, call `orchestration_finalize`. Cite only capability/procedure reuse keys that appeared in discovery. Carry material unknowns forward; `decision_ready=false` is a real blocker.
9. Use `routing_evidence` and `orchestration_evidence` when inspecting or handing evidence to validation. Routing/orchestration evidence does not replace task decisions, procedure acceptance/proof, user approval provenance, checkpoints, or protected validation where the route requires it.

## Execution and assurance contract

Fable routing and Fable validation are independent executions.

- **Ephemeral/trivial work:** no persistent task, no routing governor, no validation subagent.
- **Routine routed work:** Fable may choose only Sonnet workers and `assurance="routine"` when no hard-protected trigger applies. After decision-ready orchestration and the final checkpoint, use `task_complete_routed`; no second Fable validator is required.
- **Protected routed work:** any Opus worker automatically requires `assurance="protected"`. Hard-protected risk triggers also require protected assurance even when execution can remain Sonnet. Complete the final checkpoint, call `validation_prepare`, delegate only to `vres-os:validator` on returned Fable/high, wait for the observed review record, then call `task_complete_routed`.
- Never downgrade a protected route. If scope/risk changes materially, obtain a fresh Fable route; the service rejects protected-to-routine downgrades.
- A simple implementation can still require protected validation because consequence and complexity are separate. Conversely, hard reasoning may justify Opus and therefore protected validation even when the work is reversible.
...
Do not make the user select models. For routed persistent work, the Fable routing arbiter is the model governor: Sonnet is the default execution tier; Opus is reserved for work where deeper reasoning materially improves the result. Any Opus route requires protected Fable validation. Hard-risk work also requires protected Fable validation even if Sonnet can execute it. For non-orchestrated phase recommendations where model selection is material, `model_recommend` remains advisory; telemetry cannot auto-promote policy until independently measured paired replay exists. Never present agent-supplied scores or self-reported model identity as host measurements.
...
For routed work, the authoritative sequence is discovery -> routing_prepare -> `vres-os:routing-arbiter` -> routing_result -> plan -> governed Sonnet/Opus workers -> expert reports -> arbitration if needed -> final -> governed completion.
...
The `SubagentStop` hook records the routing arbiter, governed worker, and protected-validator model evidence. `task_complete_routed` and the database completion trigger reject a team/model/assurance mismatch.
