...
def test_explicit_validation_invalidation_tool_is_registered_through_existing_module():
    tool_text = (ROOT / "src/vres_os/validation_evidence_mcp.py").read_text(encoding="utf-8")
    entrypoint_text = (ROOT / "src/vres_os/mcp_entrypoint.py").read_text(encoding="utf-8")
    assert "def validation_invalidate(" in tool_text
    assert "ValidationLifecycleService" in tool_text
