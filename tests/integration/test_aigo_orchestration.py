...
def _task_and_session(pid: int) -> tuple[str, str]:
    repo = Repository()
    task_key = repo.begin_task(pid, "AIGO integration", "Exercise durable orchestration", "aigo-test", "chairman")
...
def test_missing_capability_is_acquired_project_locally_then_rediscovered(pg_project):
    ...
    acquired = service.acquire_project_capability(
        ...
        acquisition_evidence={"method": "bounded specialist qualification", "source": "integration-test"},
    )
    ...
    event_types = [row["event_type"] for row in service.evidence(project_id=pid, task_key=task_key)]
    assert event_types == [
...
def test_multi_expert_disagreement_and_challenger_are_preserved(pg_project):
    ...
    finance_key = discovery["capability_matches"]["financial analysis"][0]["capability_key"]
    selected = [
        {
    ...
    plan = service.record_plan(
        ...
        excluded_experts=_excluded("commercial-director", "finance-director", "challenger"),
        routing_rationale="Pricing owns the decision; Finance constrains margin; Challenger tests the conflict.",
    )
    ...
    arbitration = service.record_arbitration(
        ...
        rationale="The Challenger exposed an unmeasured elasticity assumption; protect margin while measuring it.",
    )
    final = service.finalize(
        project_id=pid,
        task_key=task_key,
        session_id=session_id,
        plan_key=plan["plan_key"],
        synthesis="Run the bounded promotion test with a margin floor and explicit elasticity measurement.",
        accepted_report_keys=[commercial["report_key"], finance["report_key"], challenge["report_key"]],
