...
def _evidence(*, provider="claude", model="candidate", input_tokens=10, output_tokens=5):
    return {
...
def test_protected_validator_cannot_be_model_experiment_target(monkeypatch):
    ...
    with pytest.raises(ValueError, match="protected validation"):
        ModelExperimentService().record_host_run(
            ...
            input_digest="a" * 64,
            output_digest="b" * 64,
            execution_evidence=_evidence(model="fable", input_tokens=1, output_tokens=1),
        )


def test_host_evidence_requires_provider_identity_and_exact_usage():
    with pytest.raises(ValueError, match="identity"):
        _host_evidence(
            ...
            input_tokens=10,
            output_tokens=5,
            execution_evidence=_evidence(model="other"),
        )
