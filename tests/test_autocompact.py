...
def test_install_sets_15_used_and_preserves_other_settings(tmp_path: Path, monkeypatch):
    ...
    assert result["used_percent"] == 15
    payload = _settings(home)
    assert payload["theme"] == "dark"
    assert payload["env"] == {"EXISTING": "yes", KEY: "15"}
...
def test_install_preserves_utf8_bom(tmp_path: Path, monkeypatch):
    ...
    assert _settings(home)["env"][KEY] == "15"
...
def test_update_migrates_owned_legacy_85_to_15(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("CLAUDE_CODE_AUTO_COMPACT_WINDOW", raising=False)
    monkeypatch.delenv("DISABLE_AUTO_COMPACT", raising=False)
    monkeypatch.delenv("DISABLE_COMPACT", raising=False)
    home = tmp_path / ".claude"
    home.mkdir()
    (home / "settings.json").write_text(
        json.dumps({"theme": "dark", "env": {KEY: "85"}}),
        encoding="utf-8",
    )
    ...
    assert result["used_percent"] == 15
    assert _settings(home) == {"theme": "dark", "env": {KEY: "15"}}


def test_existing_custom_85_is_preserved_when_not_vres_owned(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("CLAUDE_CODE_AUTO_COMPACT_WINDOW", raising=False)
    monkeypatch.delenv("DISABLE_AUTO_COMPACT", raising=False)
    monkeypatch.delenv("DISABLE_COMPACT", raising=False)
    home = tmp_path / ".claude"
    ...
    result = install_autocompact(home, owned_before=False)

    assert result["configured"] is False
    assert result["owned"] is False
    assert result["reason"] == "existing-custom-autocompact-preserved"
...
def test_remove_deletes_only_owned_15_and_preserves_other_env(tmp_path: Path):
    home = tmp_path / ".claude"
    home.mkdir()
    (home / "settings.json").write_text(
        json.dumps({"theme": "dark", "env": {"EXISTING": "yes", KEY: "15"}}),
        encoding="utf-8",
    )
...
def test_owned_threshold_is_removed_when_persistent_disable_is_added(
    ...
    home.mkdir()
    (home / "settings.json").write_text(
        json.dumps({"env": {KEY: "15", "DISABLE_AUTO_COMPACT": "1"}}),
        encoding="utf-8",
    )
...
def test_owned_threshold_is_removed_when_persistent_custom_window_is_added(
    ...
    home.mkdir()
    (home / "settings.json").write_text(
        json.dumps({"env": {KEY: "15", "CLAUDE_CODE_AUTO_COMPACT_WINDOW": "50000"}}),
        encoding="utf-8",
    )
...
def test_owned_threshold_is_removed_when_dedicated_window_is_added(
    ...
    home.mkdir()
    (home / "settings.json").write_text(
        json.dumps({"autoCompactWindow": 300000, "env": {KEY: "15"}}),
        encoding="utf-8",
    )
