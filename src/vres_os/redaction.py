...
_SECRET_KEYS = {"password", "passwd", "pwd", "apikey", "accesstoken", "refreshtoken", "token", "secret", "authorization", "clientsecret", "privatekey"}
_KV = re.compile(r'''(?i)(["']?\b(?:password|passwd|pwd|api[_ -]?key|access[_ -]?token|refresh[_ -]?token|token|secret|client[_ -]?secret)["']?\s*[:=]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,;]+)''')
...
def redact(value: Any, _depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        return {k: "[REDACTED]" if re.sub(r"[^a-z]", "", str(k).lower()) in _SECRET_KEYS
