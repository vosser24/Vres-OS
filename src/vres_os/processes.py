...
def run_bounded(args: list[str], *, cwd: Path | None = None, prompt: str = "", timeout: float = 30, max_output_bytes: int = MAX_OUTPUT_BYTES, max_result_chars: int = MAX_RESULT_CHARS, redact_output: bool = True) -> ProcessResult:
    ...
    if type(max_output_bytes) is not int or type(max_result_chars) is not int or not 1 <= max_output_bytes <= 64 * 1024 * 1024 or not 1 <= max_result_chars <= 64 * 1024 * 1024:
    ...
    with tempfile.TemporaryFile() as source, tempfile.TemporaryFile() as output:
        ...
        proc = subprocess.Popen(args, cwd=cwd, stdin=source, stdout=output, stderr=subprocess.STDOUT, **options)
...
def worker_command(module: str, *arguments: str) -> list[str]:
    ...
    import sys
    if module not in {"vres_os.cli", "vres_os.parse_worker"}:
        raise ValueError("Unapproved Vres worker module")
    loader = "import runpy,sys; sys.path.insert(0,sys.argv.pop(1)); runpy.run_module(sys.argv.pop(1),run_name='__main__')"
    return [sys.executable, "-I", "-X", "utf8", "-c", loader, str(Path(__file__).resolve().parent.parent), module, *arguments]
