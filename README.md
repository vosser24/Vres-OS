...
- AIGO-derived Claude contract ownership: the installer maintains only a three-line Vres pointer in the user-owned global `CLAUDE.md`, owns `~/.claude/vres-rules.md`, and `vres start` creates a minimal project `CLAUDE.md` only when one is absent. The universal rules explicitly prefer deterministic/simple solutions, one worker before many, and parallelism only for independent work.
- Claude Code usage-efficiency controls that stay native: a stateless model/effort/context/5h/7d status line when the host supplies those fields, focused native-compaction guidance, and engineering-only bounded-read/Bash-output discipline. The renderer performs no DB/network/transcript work and a custom existing status line is preserved.
- Project-local governed agents backed by git-tracked `.claude/agents/*.md` definitions. Vres persists source digests, role/capability ownership and write policy while keeping model selection in the trusted Sonnet/Opus envelopes.
- A minimal dependency work graph for routed experts: readiness is computed from dependencies, native Claude remains the executor, up to four governed units may run concurrently per project, failed units retry without rerunning successful siblings, and declared parallel write scopes cannot overlap. Direct Write/Edit operations by a claimed worker are restricted to its persisted scope.
- Durable work-unit acceptance contracts for implementation DAGs: write-capable units freeze bounded deterministic/judgmental criteria before execution; deterministic criteria are satisfied by structured mechanical evidence from the implementation worker; only residual judgmental criteria require a separate dependent report-only verifier. Final synthesis fails closed on unresolved acceptance, while purely deterministic work stays single-worker.
