...
| Project-agent parallel execution | Migration 033 and CI exercise project-local agent identity, dependency readiness, retry isolation, host-evidence binding and non-overlapping declared write scopes. Direct Write/Edit tools are scope-checked after host binding. CI does not prove real Claude workers overlap in time on the target Windows host, and Bash is not presented as a filesystem sandbox. | Target-Windows physical acceptance must show 2+ registered agents genuinely overlap, a dependent unit starts only after prerequisites pass, a failed sibling alone retries, scope collisions are denied, and observed model/agent/work-unit provenance is retained. |
