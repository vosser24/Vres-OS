...
`CLAUDE_CONFIG_DIR` is honored for these Claude paths. Remove a conflicting `VRES_DATA_DIR` override before using the managed Windows installer. The installer rejects unmanaged target directories, symlink/junction install roots, a legacy in-place venv, and unresolved installation journals. It does not delete other plugins or modify unrelated CLAUDE.md/settings content. If a different custom Claude `statusLine` already exists, Vres preserves it and does not take ownership. For native context rollover, Vres sets `env.CLAUDE_AUTOCOMPACT_PCT_OVERRIDE="85"` only when that setting is absent or still Vres-owned. Claude defines this percentage against its **auto-compact window**, which never exceeds the model context window; therefore the managed default compacts before the full model context falls below roughly 15% remaining. On native 1M Sonnet 5, Claude's tuned window is about 967K, so the status line can show roughly 82% full-context use when an 85%-of-window compaction fires. Existing `autoCompactWindow`/`autoCompactEnabled` settings, custom thresholds, disable flags, or `CLAUDE_CODE_AUTO_COMPACT_WINDOW` overrides are preserved rather than overwritten; Vres does not install a custom auto-compact window. Higher-precedence project/managed/launch settings can intentionally change the effective boundary.
