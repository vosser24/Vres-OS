...
**Execute:** Verify the managed user setting contains `env.CLAUDE_AUTOCOMPACT_PCT_OVERRIDE="85"` and does not contain a Vres-installed `CLAUDE_CODE_AUTO_COMPACT_WINDOW`. Run a persistent engineering task with decisions, changed files, executed tests and one unresolved next step. Exercise native `/compact` once, then exercise **automatic** compaction in a disposable session. Because Claude Code clamps `CLAUDE_CODE_AUTO_COMPACT_WINDOW` to a 100,000-token minimum, an affordable trigger test may launch one already-task-bound disposable session with a temporary command-line `--settings` JSON/file that overrides only that session to a lower `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` plus `CLAUDE_CODE_AUTO_COMPACT_WINDOW=100000`. The temporary settings file must stay outside the repository and be deleted after the test; the installed user setting must remain `85` with no persisted window override.

**Pass evidence:** The installed production setting means 85% of Claude's native auto-compact window, with no Vres-installed custom window. Because the auto-compact window is capped at the model context window, this triggers before the full context falls below roughly 15% remaining; do not require the status line to display exactly 85% at the trigger. Existing PreCompact/PostCompact Vres checkpoints and rehydration still work for both manual and auto triggers. Durable evidence records `trigger=manual|auto`. The compacted conversation retains objective/constraints, durable decisions, changed surfaces, actual test/evidence results, blockers/current work state and exact next action while dropping repeated/stale exploration and raw output already reduced to conclusions. No polling, daemon, custom summarizer, custom compactor or persisted auto-compact-window override is introduced.
